...
.focusbar{background:var(--panel2);border:1px solid var(--teal);border-radius:10px;padding:9px 14px;margin-bottom:14px;font-size:14px}
.focusbar .chip{margin-left:10px;cursor:pointer}
...
_JS = r"""
var DATA=null, findSev='all', findQ='', flowSort='bytes', flowDir='desc', flowQ='', focusHost=null;
function setFocus(h){
  focusHost=(focusHost===h)?null:h;
  var bar=$('#focusbar');
  if(bar){
    if(focusHost){bar.innerHTML='Focused on <b class="mono">'+esc(focusHost)+'</b> <button class="chip" id="clearfocus">show all</button>';
      bar.classList.remove('hidden');$('#clearfocus').onclick=function(){setFocus(focusHost);};}
...
function render(data){
  ...
  if(keys.length){
    ...
    keys.forEach(function(k){var v=hs[k];
      html+='<tr data-host="'+esc(k)+'" class="clickrow"><td class="mono">'+esc(k)+'</td><td>'+v+'</td><td><div class="bar"><span style="width:'+Math.min(100,v)+'%;background:'+scoreColor(v)+'"></span></div></td></tr>';});
    html+='</table>';
  }
...
function renderNetwork(){
  ...
  Object.keys(edges).forEach(function(k){var pr=k.split('|');if(!posByHost[pr[0]]||!posByHost[pr[1]])return;
    ...
    if(focusHost)op=(pr[0]===focusHost||pr[1]===focusHost)?0.6:0.04;else op=0.06+0.5*edges[k]/emax;
  ...
  shown.forEach(function(h){var p=posByHost[h],sc=scores[h]||0;
    ...
    if(showLabel){var lx=cx+(R+12)*Math.cos(p.a),ly=cy+(R+12)*Math.sin(p.a);
  ...
  var note=hosts.length>cap?'<p class="note">Showing the '+cap+' busiest hosts of '+hosts.length+'. Click a host to focus. Node size is total traffic; edges are conversations.</p>':'<p class="note">Click a host to focus. Node size is total traffic; edges are conversations.</p>';
